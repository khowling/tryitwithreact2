...
module.exports = function (connect) {
    ...
    class MongoStore extends Store {
      constructor(options) {
        ...
        if (options.db && options.db.listCollections) {
      ...
      setAutoRemoveAsync() {
        return this.collection.createIndex({"_ts":1}, {expireAfterSeconds: 60*60*8})
      }
      ...
      set(sid, session, callback) {
        ...
        try {
          s = {partition_key: 0, _id: this.computeStorageId(sid), session: this.transformFunctions.serialize(session)}
        } catch (err) {
          return callback(err)
        }
  
        if (session && session.cookie && session.cookie.expires) {
          s.ttl = (new Date(session.cookie.expires) - new Date()) / 1000
        } else {
        ...
        return withCallback(this.collectionReady()
          .then(collection => collection.updateOne({partition_key: 0, _id: this.computeStorageId(sid)}, {$set: s}, {upsert: true}))
          .then(rawResponse => {
      ...
      touch(sid, session, callback) {
        ...
        return withCallback(this.collectionReady()
          .then(collection => collection.updateOne({partition_key: 0, _id: this.computeStorageId(sid)}, {$set: updateFields}))
          .then(result => {
